...
// CompactionWithinLevel is responsible for executing the compaction. Each (manual or
// automated) compaction corresponds to a CompactionWithinLevel object, and usually
// goes through the stages of `Prepare()`->`Run()`->`Install()`. CompactionWithinLevel
// will divide the compaction into subcompactions and execute them in parallel
// if needed.
class CompactionWithinLevel {
 public:
  CompactionWithinLevel(
  ...
  // no copy/move
  CompactionWithinLevel(CompactionWithinLevel&& job) = delete;
  CompactionWithinLevel(const CompactionWithinLevel& job) = delete;
  CompactionWithinLevel& operator=(const CompactionWithinLevel& job) = delete;
